...
async def on_member_join(member):
    ...
    roles = list(
        filter(lambda r: r, map(lambda r: get(member.guild.roles, name=r), roles))
    )
    ...
    response = await channel.send(
        f"{member.mention} Seja bem vindo(a), meu {random.choice(VOCATIVES)}!"
    )
...
async def on_message(message):
    ...
    if message.content.lower() == "bom dia":
        ...
        response = await message.reply(
            f"Bom dia flor do dia, {message.author.mention}!"
        )
        await reactToResponse(
            bot, response, emojiList=["🌻"]
        )  # Reacts to the response message
    ...
    if message.content.lower() == "boa noite":
        ...
        # Sends response message
        response = await message.reply(
            f"Boa noite flor da noite, {message.author.mention}!"
        )
        await reactToResponse(
            bot, response, emojiList=["🌼"]
        )  # Reacts to the response message
    ...
    if message.content.lower() == "boa tarde":
        ...
        # Sends response message
        response = await message.reply(
            f"Boa tarde flor da tarde, {message.author.mention}!"
        )
        await reactToResponse(
            bot, response, emojiList=["💐"]
        )  # Reacts to the response message
    ...
    if message.content.lower() in ["oi", "oi arrombado"]:
        ...
        await reactToMessage(
            bot,
            message,
            ["🦆"] if "Bixos" in roles or "Convidado(a)" in roles else ["💩"],
        )
        ...
        response = await message.reply(
            f"{output}, {message.author.mention}!",
            file=(discord.File(img) if img else img),
        )
    ...
    if message.content.lower() in [
        ...
        response = await message.channel.send(
            ...
            file=discord.File("./images/estou-devastado.png"),
        )
        await reactToResponse(
            bot, response, emojiList=["😿"]
        )  # Reacts to the response message
    ...
    if message.content.lower() in ["auau", "au au", "nautau", ">arthur"]:
        ...
        # Sends response message
        response = await message.channel.send(
            f"feliz nAUtAU", file=discord.File("./images/todão-arthur.png")
        )
        await reactToResponse(
            bot, response, emojiList=["🐶"]
        )  # Reacts to the response message
    ...
    for element in triggerSheet:
        if message.content and message.content.lower() in [
            trigger for trigger in element["TRIGGER"].split("\n") if trigger
        ]:
            print(
            ...
            if images:
                ...
                response = await message.channel.send(
                    content=element["RESPONSE TEXT"], tts=tts
                )
...
@bot.command(
    brief="Atualiza os comandos a partir da planilha.", aliases=["atualizar", "update"]
)
async def refresh(ctx):
    ...
    if not isEmpty:
        ...
        response = await ctx.send(
            "Os comandos e triggers foram atualizados com sucesso."
        )
        print("   [**] The response was successfully sent.")
        await reactToResponse(bot, response)
...
async def cadê(ctx, *user):
    ...
    user = " ".join(user)
    response = await ctx.send(
        f"**cadê vc otário??** {user}", file=discord.File("./images/cade-vc.png")
    )
...
async def clear(ctx):
    ...
    def filterFunction(m):
        ...
        isImportant = (
            search(r"`\[ABERTURA DE PROJETOS\]`\n\n", m.content)
            or search(r"`\[PRESENÇA DE REUNIÃO\]`\n\n", m.content)
            or search(r"`\[VOTAÇÃO\]`\n\n", m.content)
        )
...
async def send(ctx, *argv):
    ...
    if len(argv) != 2 or not search('[^"]', argv[0]) or not search('[^"]', argv[1]):
        ...
        channel = get(ctx.guild.text_channels, name=channelRef) or get(
            ctx.guild.text_channels, mention=channelRef
        )
        ...
        # if the channel does not exist
            ...
            response = (
                f"Infelizmente o canal passado (`{channelRef}`) não foi encontrado."
            )
